...
abstract class EncodeHelper implements HelperCore {
  ...
  void _writeToJsonSimple(StringBuffer buffer, Iterable<FieldElement> fields) {
    /* final bypassKeysToJson = config.bypassKeysToJson ?? false;
    ...
    for (var field in fields) {
      ...
      final excludeToJson = jsonKey.excludeToJson;
      final access = _fieldAccess(field);
      if (bypassKeysToJson || excludeToJson) {
        buffer.write(
            '=> ${_serializeField(field, access)} as Map<String, dynamic>;');
        break;
      } else {
        buffer.write(
